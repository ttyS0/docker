...
I copied directly from the [external-storage](https://github.com/kubernetes-incubator/external-storage) repository, and changed a few things around. Most notably is moving the provisioner to the `luminous` release of ceph, and adding additional rbd features (exclusive-lock,object-map,fast-diff). In order for the extra features to work, `rbd-nbd` needs to be installed on clients if their kernels won't support those features. This happens to be the case with ARM64 clients. I push the rbd-provisoner container to my internal docker repository, and then deploy via the regular kubernetes deployment mechanism. 
